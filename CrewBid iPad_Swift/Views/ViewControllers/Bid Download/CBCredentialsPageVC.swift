                    ...
                    DispatchQueue.main.async{
                        self.dismiss(animated: false) {
                            if self.awardsViewModel?.bidPeriod.awardString != nil {
                                ...
                                self.awardsViewModel?.getAwardAlertFromServer(empNum: emp) { title, message, shouldOpenAward in
...
//        }
    ...
    func showAwardAlertFromLocal(empNum: String) {
        ...
        if userAward == nil {
                userAward = awards.first(where: { $0.empNum == rawEmpNum && $0.type == "Reserve" })
            }
