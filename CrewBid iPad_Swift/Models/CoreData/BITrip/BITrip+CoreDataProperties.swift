...
extension BITrip : Identifiable {
    ...
    func tripText() -> String {
        // Generate text description for the trip, considering whether it's for a Flight Attendant or not
        return ""
...
//                let equipment = legInfo.equipment!
//                
...
//                    }
//                    else {
//                        textForTrip += "\(df.string(from: departDate!)) \(deadHead ? "DH" : "  ")\(formattedLegFlight) \(departCity) \(departFormatter.string(from: departDate!)) \(arriveCity) \(arriveFormatter.string(from: arriveDate!)) \(equipment)00 \(blockTime) \(groundTime) \((legInfo.isAircraftChange?.boolValue)! ? "acft change" : "           ") \(String(format:"%03.0f", (legInfo.pay?.floatValue)! * 100.0))\n"
//                    }
//                }
//                else{
//                    textForTrip += "\(df.string(from: departDate!)) \(deadHead ? "DH" : "  ")\(formattedLegFlight) \(departCity) \(departFormatter.string(from: departDate!)) \(arriveCity) \(arriveFormatter.string(from: arriveDate!)) \(equipment) \(blockTime) \(groundTime) \((legInfo.isAircraftChange?.boolValue)! ? "acft change" : "           ") \(String(format:"%03.0f", (legInfo.pay?.floatValue)! * 100.0))\n"
...
//                } else {
//                    groundMinutes -= 2 * Int(truncating: (trip.info?.debriefMinutes!)!)  // groundMinutes is duty break in CrewBid Web
//                    groundHours = groundMinutes / 60
//                    groundTime = String(format: "%2zd%02zd", Int(groundHours), Int(groundMinutes) % 60)
//                }
...
//            textForTrip += String(format:"%8sTAFB %@%20sTotals %@ %@  %4.0f",
//                                     " ",
//                                     tafbTime,
//                                     " ",
//                                     blockTime,
//                                     dutyTime,
//                                     (isFA ? trip.info?.faPay?.floatValue ?? 0 : trip.info?.jsonPay?.floatValue ?? 0) * 100.0)
//        } else {
//            textForTrip += String(format:"%8sTAFB %@%20sTotals %@ %@  %4.0f",
//                                     " ",
//                                     tafbTime,
...
//                                     blockTime,
//                                     dutyTime,
//                                     (trip.info?.faPay?.floatValue ?? 0) * 100.0)
//        }
//        return textForTrip
//    }
}
